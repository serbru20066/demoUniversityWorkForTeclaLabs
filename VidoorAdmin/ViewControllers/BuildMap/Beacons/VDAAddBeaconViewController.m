...
@interface VDAAddBeaconViewController ()

@property (nonatomic, assign) bool animated;

@end

@implementation VDAAddBeaconViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self customNavigationWithTitleInFirstLine:@"Agregar mi" andSecondLine:@"iBeacon" withColor:[UIColor whiteColor]];
...
- (IBAction)addBeaconAction:(id)sender {
    if ([self isFormValid]) {
        [self addBeaconOperation];
    }
}
...
- (void)addBeaconOperation {
    [self initSpinnerAnimation];
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    NSDictionary *parameters = @{@"Major" : self.majorTextField.text,
...
- (void)presentAlertViewConfirmationRequest {
    self.alertView = [[JTAlertView alloc] initWithTitle:[@"Se agregó un nuevo iBeacon de manera exitosa" uppercaseString] andImage:[UIImage imageNamed:@"menu_beacon"]];
