...
#import "VDAAddPlaceViewController.h"
#import "ImgurAnonymousAPIClient.h"
#import "Constants.pch"

@interface VDAAddPlaceViewController ()

@property (nonatomic, assign) bool animated;

@end

@implementation VDAAddPlaceViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self customNavigationWithTitleInFirstLine:@"Agregar" andSecondLine:@"Establecimiento" withColor:[UIColor whiteColor]];
...
- (void)addPlaceOperation {
    [self initSpinnerAnimation];
    UIImage *image = self.imagePlaceImageView.image;
    [[ImgurAnonymousAPIClient client] uploadImage:image
                                ...
                                completionHandler:^(NSURL *imgurURL, NSError *error) {
                                    ...
                                    NSDictionary *parameters = @{@"nom_estableci" : self.namePlaceTextField.text,
                                    ...
                                    NSString *url = [NSString stringWithFormat:@"%@%@",urlBase,@"/Establecimientoes"];
...
- (void)addTapGestureRecognizer {
    [self.imagePlaceImageView setUserInteractionEnabled:YES];
    UITapGestureRecognizer* singleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleSingleTap:)];
    singleTap.numberOfTapsRequired = 1;
    [self.imagePlaceImageView addGestureRecognizer:singleTap];
}
...
- (void)assetsPickerController:(CTAssetsPickerController *)picker didFinishPickingAssets:(NSArray *)assets {
    ...
    [manager requestImageDataForAsset:asset
                        ...
                        resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info)
     {
         ...
         self.imagePlaceImageView.image = image;
         [self.imagePlaceImageView.layer setCornerRadius:40.0f];
         [self.imagePlaceImageView.layer setMasksToBounds:YES];
     }];
}
...
- (void)presentAlertViewConfirmationRequest {
    self.alertView = [[JTAlertView alloc] initWithTitle:[@"Se agregó un nuevo establecimiento de manera exitosa" uppercaseString] andImage:[UIImage imageNamed:@"menu_map"]];
